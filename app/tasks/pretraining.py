...
class PretrainingTask(BaseTask):
    """
    Task handler for Domain-Adaptive Continued Pretraining (DAPT).
    Implements full lifecycle: prepare -> validate -> format -> tokenize -> build_labels.
    """
    def __init__(self):
        super().__init__(name="pretraining")
    ...
    def validate(self, prepared: Dict[str, Any], ctx: Optional[Any] = None, **kwargs) -> bool:
        """Checks text exists before formatting."""
        is_valid = bool(prepared.get("text"))
        if not is_valid and ctx and hasattr(ctx, "log"):
            ctx.log("PretrainingTask: Skipped sample due to empty text string.", level="debug")
        return is_valid

    def format(self, prepared: Dict[str, Any], formatter: Any, ctx: Optional[Any] = None, **kwargs) -> str:
        if hasattr(formatter, "render"):
            return formatter.render(prepared, format_type="pretraining", **kwargs)
...
task_manager.register("pretraining", PretrainingTask())
